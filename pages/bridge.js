...
import IplnBridgeLayout from 'layouts/IplnBridgeLayout';
...
// add input for expected slippage amount to complete swap!
    ...
    // (Moralis.connector.provider, UsertransactionInput,handleSuccess,handleError,setTransactionPopulated, setTxDetails,Moralis.chainId? parseInt(Moralis.chainId): 0) // adding chainId
    (Moralis.connector.provider, UsertransactionInput,handleSuccess,handleError,setTransactionPopulated, setTxDetails,Moralis.chainId? parseInt(Moralis.chainId): 0)
    };
    ...
    await paymentTypeLogicExecutor(txDetails,setTransacting,Moralis.connector.provider,handleSuccess,handleError, setTransactionPopulated, setTxDetails, setTxReciept, Moralis.chainId? parseInt(Moralis.chainId): 0)
    };
    ...
    useEffect(()=>{
       ...
       try{ 
        ...
        const loadUsersBalances = async () => {
            ...
            if(sendersToken == Utils.EthAddress){
              setSendersTokenBalance(await Utils.getUserNativeBalance(account, Moralis.chainId? parseInt(Moralis.chainId): 0))
            }else{
              setSendersTokenBalance(await Utils.getUserErc20Balance(sendersToken,account, Moralis.chainId? parseInt(Moralis.chainId): 0))
            }
          } catch(e){
            return
          }
      ...
      (async function() {
        ...
        if(await PaymentInputValidator(UsertransactionInput,setValidationResponce, setvalidatingInput,Moralis.chainId? parseInt(Moralis.chainId): 0)){
          // if(true){
      ...
      }, [sendersToken, reciversToken,amountToBeSent, userSlippage, quote, account, address, reciver,Moralis.chainId]);
