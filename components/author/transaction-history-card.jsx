...
export default function TransactionHistoryCard({ item }) {
    ...
    return (<div className="rounded-lg bg-white p-4 text-sm shadow-card dark:bg-light-dark sm:p-5 md:p-6">
      <div className="flex items-center justify-between border-b border-dashed border-gray-200 pb-3.5 dark:border-gray-700 sm:pb-5">
        <div className="flex items-center font-medium ">
          <Image src={avatar} alt="wallet" width={24} height={24} placeholder="blur" className="rounded-full"/>
      ...
      <div className="grid grid-cols-9 gap-x-3 pt-4 md:gap-x-5 md:pt-6">
        <div className="col-span-4 flex flex-col gap-2.5 sm:flex-row sm:gap-x-4 md:flex-col 2xl:flex-row">
          <div className="flex items-center lg:w-1/2">
            <div className="relative flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-white md:h-9 md:w-9 xl:h-10 xl:w-10" style={{ backgroundColor: bgColor }}>
              <LongArrowUp className={`h-5 w-5 xl:h-6 xl:w-6 ${transactionType === 'credited' ? 'rotate-180' : 'rotate-0'}`}/>
          ...
          <div className="flex items-center lg:w-1/2">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gray-600/5 text-gray-600 dark:text-gray-400 md:h-9 md:w-9 xl:h-10 xl:w-10">
              {transactionFromAvatar ? (<Image src={transactionFromAvatar} alt={transactionFrom} width={40} height={40} className="rounded-full"/>) : (<QuestionIcon className="h-5 w-5 lg:h-6 lg:w-6"/>)}
            </div>
            <div className="flex flex-col truncate ltr:ml-2.5 rtl:mr-2.5 xl:ltr:ml-4 xl:rtl:mr-4">
        ...
        <div className="col-span-4  flex flex-col gap-2.5 sm:flex-row sm:gap-x-4 md:flex-col 2xl:flex-row">
          <div className="flex items-center lg:w-1/2">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gray-600/5 text-gray-600 dark:text-gray-400 md:h-9 md:w-9 xl:h-10 xl:w-10">
              {transactionMethodLogo ? (<Image src={transactionMethodLogo} alt={transactionMethod} width={40} height={40} className="rounded-full" placeholder="blur"/>) : (<QuestionIcon className="h-5 w-5 xl:h-6 xl:w-6"/>)}
            </div>
            <div className="flex flex-col truncate ltr:ml-2.5 rtl:mr-2.5 xl:ltr:ml-4 xl:rtl:mr-4">
