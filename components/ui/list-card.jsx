...
function handleImageSize(variant) {
    let size = 0;
    if (variant === 'large') {
        size = 40;
    }
...
export default function ListCard({ item, className = 'p-3 tracking-wider rounded-lg sm:p-4', variant = 'small', }) {
      ...
      <div className="flex items-center">
       ...
       (<div className={cn('rounded-full', variants[variant])}>
        ...
        <div className="ltr:ml-2 rtl:mr-2">
          {name}
          {coinType && (<span className="block pt-0.5 text-xs font-normal capitalize text-gray-600 dark:text-gray-400">
              {coinType}
            </span>)}
        </div>
      </div>
