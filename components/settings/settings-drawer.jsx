...
function ThemeSwitcher() {
    ...
    return (<div className="px-6 pt-8">
      <h4 className="mb-4 text-sm font-medium text-gray-900 dark:text-white">
        Mode
      </h4>
      <RadioGroup value={theme} onChange={setTheme} className="grid grid-cols-2 gap-5 ">
        <RadioGroup.Option value="light">
          {({ checked }) => (<div className="group cursor-pointer">
              <span className={`flex h-[70px] items-center justify-center rounded-lg text-center text-sm font-medium uppercase tracking-wide transition-all ${checked
              ...
              <span className={`mt-3 block text-center text-sm transition-all ${checked
...
function LayoutSwitcher() {
    const [layout, setLayout] = useLocalStorage('map3-layout', 'ltr');
    useDirection(layout ? layout : 'ltr');
    return (<div className="px-6 pt-8">
      <h4 className="mb-4 text-sm font-medium text-gray-900 dark:text-white">
        Layout
      </h4>
      <RadioGroup value={layout} onChange={setLayout} className="grid grid-cols-2 gap-5 ">
...
function ColorSwitcher() {
    const [themeColor, setThemeColor] = useLocalStorage('map3-color', '#323743');
    useThemeColor(themeColor ? themeColor : '#323743');
    return (<div className="px-6 pt-8">
      <h4 className="mb-4 text-sm font-medium text-gray-900 dark:text-white">
        Color
      </h4>
      <RadioGroup value={themeColor} onChange={setThemeColor} className="grid grid-cols-3 gap-5 ">
        {ColorPreset.map((item, index) => (<RadioGroup.Option value={item.value} key={index}>
            {({ checked }) => (<div className="group cursor-pointer">
                <span className={`flex h-[70px] items-center justify-center rounded-lg text-center text-sm font-medium uppercase tracking-wide transition-all ${checked
                    ? 'bg-white shadow-large dark:bg-gray-600'
                    : 'bg-gray-100 text-gray-500 group-hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-400 dark:group-hover:bg-gray-700'}`}>
                  <span className="h-8 w-8 rounded-full" style={{ backgroundColor: item.value }}/>
                </span>
                <span className={`mt-3 block text-center text-sm transition-all ${checked
                    ? 'text-brand dark:text-white'
                    : 'text-gray-500 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-white'}`}>
...
export default function SettingsDrawer() {
    ...
    return (<Transition appear show={isSettingsOpen} as={Fragment}>
      <Dialog as="div" className="fixed inset-0 z-40 overflow-hidden" onClose={closeSettings}>
          ...
          <div className="fixed inset-y-0 w-80 max-w-full bg-white/95 shadow-[0_0_80px_rgba(17,24,39,0.2)] backdrop-blur ltr:right-0 rtl:left-0 dark:bg-dark/90">
            ...
            <div className="h-full w-full">
              <div className="flex h-16 items-center justify-between gap-6 border-b border-dashed border-gray-200 px-6 dark:border-gray-700">
                ...
                <Button title="Close" color="white" shape="circle" variant="transparent" size="small" onClick={closeSettings}>
